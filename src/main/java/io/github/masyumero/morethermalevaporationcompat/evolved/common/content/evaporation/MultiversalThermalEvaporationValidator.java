...
import io.github.masyumero.morethermalevaporationcompat.evolved.common.registries.MoreThermalEvaporationCompatBlockTypes;
...
public class MultiversalThermalEvaporationValidator extends CuboidStructureValidator<MultiversalThermalEvaporationMultiblockData> {

    private static final VoxelCuboid MIN_CUBOID = new VoxelCuboid(4, 3, 4);
    private static final VoxelCuboid MAX_CUBOID = new VoxelCuboid(4, 4098, 4);

    private boolean foundController = false;

    @Override
    protected FormationResult validateFrame(FormationProtocol<MultiversalThermalEvaporationMultiblockData> ctx, BlockPos pos, BlockState state, CasingType type, boolean needsFrame) {
        boolean controller = structure.getTile(pos) instanceof TileEntityMultiversalThermalEvaporationController;
    ...
    protected CasingType getCasingType(BlockState state) {
        ...
        if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.MULTIVERSAL_THERMAL_EVAPORATION_BLOCK)) {
            return CasingType.FRAME;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.MULTIVERSAL_THERMAL_EVAPORATION_VALVE)) {
            return CasingType.VALVE;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.MULTIVERSAL_THERMAL_EVAPORATION_CONTROLLER)) {
    ...
    @Override
    public FormationResult postcheck(MultiversalThermalEvaporationMultiblockData structure, Long2ObjectMap<ChunkAccess> chunkMap) {
        if (!foundController) {
