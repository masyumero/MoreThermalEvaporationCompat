...
public class OverclockedThermalEvaporationValidator extends CuboidStructureValidator<OverclockedThermalEvaporationMultiblockData> {

    private static final VoxelCuboid MIN_CUBOID = new VoxelCuboid(4, 3, 4);
    private static final VoxelCuboid MAX_CUBOID = new VoxelCuboid(4, LoadConfig.MTE_COMPAT_CONFIG.getHeight(EMBaseTier.OVERCLOCKED), 4);

    private boolean foundController = false;

    @Override
    protected FormationResult validateFrame(FormationProtocol<OverclockedThermalEvaporationMultiblockData> ctx, BlockPos pos, BlockState state, CasingType type, boolean needsFrame) {
        boolean controller = structure.getTile(pos) instanceof TileEntityOverclockedThermalEvaporationController;
    ...
    protected CasingType getCasingType(BlockState state) {
        ...
        if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.OVERCLOCKED_THERMAL_EVAPORATION_BLOCK)) {
            return CasingType.FRAME;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.OVERCLOCKED_THERMAL_EVAPORATION_VALVE)) {
            return CasingType.VALVE;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.OVERCLOCKED_THERMAL_EVAPORATION_CONTROLLER)) {
    ...
    @Override
    public FormationResult postcheck(OverclockedThermalEvaporationMultiblockData structure, Long2ObjectMap<ChunkAccess> chunkMap) {
        if (!foundController) {
