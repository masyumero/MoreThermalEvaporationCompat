...
public class DenseThermalEvaporationValidator extends CuboidStructureValidator<DenseThermalEvaporationMultiblockData> {
    ...
    @Override
    protected FormationResult validateFrame(FormationProtocol<DenseThermalEvaporationMultiblockData> ctx, BlockPos pos, BlockState state, CasingType type, boolean needsFrame) {
        boolean controller = structure.getTile(pos) instanceof TileEntityDenseThermalEvaporationController;
    ...
    protected CasingType getCasingType(BlockState state) {
        ...
        if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.DENSE_THERMAL_EVAPORATION_BLOCK)) {
            return CasingType.FRAME;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.DENSE_THERMAL_EVAPORATION_VALVE)) {
            return CasingType.VALVE;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.DENSE_THERMAL_EVAPORATION_CONTROLLER)) {
    ...
    @Override
    public FormationResult postcheck(DenseThermalEvaporationMultiblockData structure, Long2ObjectMap<ChunkAccess> chunkMap) {
        if (!foundController) {
