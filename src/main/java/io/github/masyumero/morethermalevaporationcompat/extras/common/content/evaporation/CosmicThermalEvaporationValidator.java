...
public class CosmicThermalEvaporationValidator extends CuboidStructureValidator<CosmicThermalEvaporationMultiblockData> {

    private static final VoxelCuboid MIN_CUBOID = new VoxelCuboid(4, 3, 4);
    private static final VoxelCuboid MAX_CUBOID = new VoxelCuboid(4, LoadConfig.MTE_COMPAT_CONFIG.getHeight(AdvancedTier.COSMIC), 4);

    private boolean foundController = false;

    @Override
    protected FormationResult validateFrame(FormationProtocol<CosmicThermalEvaporationMultiblockData> ctx, BlockPos pos, BlockState state, CasingType type, boolean needsFrame) {
        boolean controller = structure.getTile(pos) instanceof TileEntityCosmicThermalEvaporationController;
    ...
    protected CasingType getCasingType(BlockState state) {
        ...
        if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.COSMIC_THERMAL_EVAPORATION_BLOCK)) {
            return CasingType.FRAME;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.COSMIC_THERMAL_EVAPORATION_VALVE)) {
            return CasingType.VALVE;
        } else if (BlockType.is(block, MoreThermalEvaporationCompatBlockTypes.COSMIC_THERMAL_EVAPORATION_CONTROLLER)) {
    ...
    @Override
    public FormationResult postcheck(CosmicThermalEvaporationMultiblockData structure, Long2ObjectMap<ChunkAccess> chunkMap) {
        if (!foundController) {
